...
async function init() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS categories (id SERIAL PRIMARY KEY, name_ar TEXT NOT NULL, name_en TEXT NOT NULL, sort_order INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
    CREATE TABLE IF NOT EXISTS items (id SERIAL PRIMARY KEY, category_id INTEGER NOT NULL REFERENCES categories(id) ON DELETE CASCADE, name_ar TEXT NOT NULL, name_en TEXT NOT NULL, ingredients_ar TEXT NOT NULL DEFAULT '', ingredients_en TEXT NOT NULL DEFAULT '', price INTEGER NOT NULL, image_path TEXT, is_popular INTEGER DEFAULT 0, sort_order INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
    CREATE TABLE IF NOT EXISTS admin (id SERIAL PRIMARY KEY, username TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
...
module.exports = async function handler(req, res) {
  ...
  if (cm) {
    ...
    if (m === 'PUT') {
      ...
      const { name_ar, name_en, sort_order } = req.body || {}
      const u2 = [], v = []; let i = 1
      if (name_ar !== undefined) { u2.push(`name_ar=$${i++}`); v.push(name_ar) }
      if (name_en !== undefined) { u2.push(`name_en=$${i++}`); v.push(name_en) }
      if (sort_order !== undefined) { u2.push(`sort_order=$${i++}`); v.push(sort_order) }
      if (!u2.length) return res.status(400).json({ error: 'No fields' })
      u2.push('updated_at=CURRENT_TIMESTAMP'); v.push(id)
      await ex(`UPDATE categories SET ${u2.join(',')} WHERE id=$${i}`, v)
  ...
  if (u === '/api/items' && m === 'POST') {
    ...
    if (!name_ar || !name_en || price === undefined || !category_id) return res.status(400).json({ error: 'Missing' })
    return res.status(201).json((await q('INSERT INTO items (category_id,name_ar,name_en,ingredients_ar,ingredients_en,price,image_path,sort_order) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *', [category_id, name_ar, name_en, ingredients_ar||'', ingredients_en||'', Number(price), image_path||null, sort_order??0]))[0])
  }
  ...
  if (im) {
    ...
    if (m === 'PUT') {
      ...
      if (name_ar !== undefined) { u2.push(`name_ar=$${i++}`); v.push(name_ar) }
      if (name_en !== undefined) { u2.push(`name_en=$${i++}`); v.push(name_en) }
      if (ingredients_ar !== undefined) { u2.push(`ingredients_ar=$${i++}`); v.push(ingredients_ar) }
      if (ingredients_en !== undefined) { u2.push(`ingredients_en=$${i++}`); v.push(ingredients_en) }
      if (price !== undefined) { u2.push(`price=$${i++}`); v.push(Number(price)) }
      if (category_id !== undefined) { u2.push(`category_id=$${i++}`); v.push(Number(category_id)) }
      if (image_path !== undefined) { u2.push(`image_path=$${i++}`); v.push(image_path) }
      if (is_popular !== undefined) { u2.push(`is_popular=$${i++}`); v.push(is_popular ? 1 : 0) }
      if (sort_order !== undefined) { u2.push(`sort_order=$${i++}`); v.push(Number(sort_order)) }
      if (!u2.length) return res.status(400).json({ error: 'No fields' })
      u2.push('updated_at=CURRENT_TIMESTAMP'); v.push(id)
      await ex(`UPDATE items SET ${u2.join(',')} WHERE id=$${i}`, v)
