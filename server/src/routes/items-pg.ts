...
const upload = multer({ storage, fileFilter: imageFilter, limits: { fileSize: MAX_FILE_SIZE } })
...
router.get('/', async (req: Request, res: Response) => {
  ...
  if (categoryId) {
    ...
    rows = await query('SELECT * FROM items WHERE category_id = $1 ORDER BY sort_order ASC', [catId])
  } else {
...
router.post('/', authMiddleware, handleUpload, async (req: Request, res: Response) => {
  const { name_ar, name_en, ingredients_ar, ingredients_en, price, category_id, sort_order, is_popular } = req.body
  ...
  const category = await queryOne('SELECT id FROM categories WHERE id = $1', [catId])
  if (!category) {
    if (req.file) deleteFileIfExists(req.file.path)
  ...
  const image_path: string | null = req.file ? `/uploads/${req.file.filename}` : null
  ...
  try {
    const result = await query(
      ...
      [catId, name_ar, name_en, ingredients_ar, ingredients_en, priceInt, image_path, isPopular, sortOrder]
    )
...
router.put('/:id', authMiddleware, handleUpload, async (req: Request, res: Response) => {
  const id = Number(req.params.id)
  if (Number.isNaN(id)) {
    if (req.file) deleteFileIfExists(req.file.path)
    res.status(400).json({ error: 'Invalid item id' })
  ...
  const existing = await queryOne('SELECT * FROM items WHERE id = $1', [id]) as any
  if (!existing) {
    if (req.file) deleteFileIfExists(req.file.path)
    res.status(404).json({ error: 'Item not found' })
    return
  ...
  if (name_ar !== undefined) { updates.push(`name_ar = $${paramIndex++}`); values.push(name_ar) }
  if (name_en !== undefined) { updates.push(`name_en = $${paramIndex++}`); values.push(name_en) }
  if (ingredients_ar !== undefined) { updates.push(`ingredients_ar = $${paramIndex++}`); values.push(ingredients_ar) }
  if (ingredients_en !== undefined) { updates.push(`ingredients_en = $${paramIndex++}`); values.push(ingredients_en) }
  if (priceInt !== undefined) { updates.push(`price = $${paramIndex++}`); values.push(priceInt) }
  if (category_id !== undefined) { updates.push(`category_id = $${paramIndex++}`); values.push(Number(category_id)) }
  if (req.file) { updates.push(`image_path = $${paramIndex++}`); values.push(imagePath) }
  if (is_popular !== undefined) { updates.push(`is_popular = $${paramIndex++}`); values.push(is_popular === 'true' || is_popular === '1' ? 1 : 0) }
  ...
  try {
    await query(`UPDATE items SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, values)
  } catch (dbErr) {
    if (req.file) deleteFileIfExists(req.file.path)
    throw dbErr
  }
  ...
  const updated = await queryOne('SELECT * FROM items WHERE id = $1', [id])
  res.json(updated)
})
...
router.delete('/:id', authMiddleware, async (req: Request, res: Response) => {
  ...
  const item = await queryOne('SELECT * FROM items WHERE id = $1', [id]) as any
  if (!item) {
    res.status(404).json({ error: 'Item not found' })
    return
  }
